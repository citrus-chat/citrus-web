...
class IndexedDbCryptoStorage implements ICryptoStorage{
    ...
    async getConversationKey(conversationId: string,keyVersion: number): Promise<IConversationKey | null> {
    ...
    async getLatestConversationKey(conversationId: string): Promise<IConversationKey | null> {

        const db = await citrusDb;

        const index = db.transaction("conversationKeys").store.index("conversationId");

        const keys =
            await index.getAll(conversationId);
