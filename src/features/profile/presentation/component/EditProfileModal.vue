...
async function saveChanges() {
  ...
  setTimeout(() => { saveSuccess.value = false; emit("update:visible", false); }, 700);
}

// ── Unsaved guard ─────────────────────────────────────────────────────────────
const showUnsaved = ref(false);
function tryClose() {
  if (hasChanges.value) { showUnsaved.value = true; return; }
...
// ── Privacy field list ────────────────────────────────────────────────────────
type PrivacyBoolKey = keyof Omit<ProfileFormData["privacySettings"], "allowGroupInvites">;
interface PrivacyField { key: PrivacyBoolKey; label: string; icon: string; }
      ...
      >
        ...
        >
          ...
          <div class="flex items-center justify-between px-6 pt-6 pb-4 shrink-0">
            ...
            <button
              ...
              class="flex h-8 w-8 items-center justify-center rounded-full border transition
                     ...
                     dark:border-white/10 dark:bg-slate-800 dark:text-slate-400 dark:hover:text-slate-200"
              @click="tryClose"
            >
          ...
          <div class="flex gap-1 px-6 pb-4 shrink-0">
            <button
              v-for="t in [{ id: 'profile', label: 'Perfil', icon: 'pi-user' }, { id: 'privacy', label: 'Privacidad', icon: 'pi-lock' }]"
              :key="t.id"
              type="button"
          ...
          <div class="flex-1 overflow-y-auto px-6 pb-2 space-y-5">
            ...
            <template v-if="tab === 'profile'">
              ...
              <!-- Username -->
              <div>
                <label class="block text-xs font-semibold uppercase tracking-[0.15em] text-slate-500 dark:text-slate-400 mb-2">Username</label>
                <div class="relative">
                  <span class="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 text-sm select-none">@</span>
                  <input
                    v-model="localForm.username"
                    type="text"
                    class="w-full rounded-xl border pl-7 pr-4 py-2.5 text-sm outline-none transition
              ...
              <div>
                <label class="block text-xs font-semibold uppercase tracking-[0.15em] text-slate-500 dark:text-slate-400 mb-2">
                  Email corporativo
                  <span class="normal-case tracking-normal font-normal text-slate-400 dark:text-slate-500">· no editable</span>
                </label>
                <div class="relative">
                  <i class="pi pi-envelope absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 text-sm" />
                  <div
                    class="w-full rounded-xl border pl-9 pr-4 py-2.5 text-sm
                              border-slate-200/60 bg-slate-100 text-slate-500
                              dark:border-white/5 dark:bg-slate-800/40 dark:text-slate-500
              ...
              <!-- Descripción -->
              <div>
                <label class="block text-xs font-semibold uppercase tracking-[0.15em] text-slate-500 dark:text-slate-400 mb-2">Descripción</label>
                <textarea
                  v-model="localForm.description"
                  rows="4"
                  maxlength="200"
                  class="w-full rounded-xl border px-4 py-3 text-sm outline-none resize-none transition
                         border-slate-200 bg-slate-50 text-slate-900
                         dark:border-white/10 dark:bg-slate-800 dark:text-slate-100
                         focus:border-orange-400 focus:ring-2 focus:ring-orange-400/20"
            ...
            <template v-if="tab === 'privacy'">
              ...
              <div>
                <p class="text-xs font-semibold uppercase tracking-[0.15em] text-slate-500 dark:text-slate-400 mb-1">
                  Visibilidad de datos
                </p>
                <p class="text-xs text-slate-400 dark:text-slate-500 mb-3 leading-relaxed">
                  Los admins y tu jefe directo siempre pueden ver todos tus datos.
                </p>
                <div class="space-y-2">
                  <div
                    ...
                    :class="localForm.privacySettings[field.key]
                    ...
                    @click="localForm.privacySettings[field.key] = !localForm.privacySettings[field.key]"
                  >
                    <!-- Icon -->
                    <div
                    ...
                    >
                      <i
                        ...
                        :class="[field.icon, localForm.privacySettings[field.key]
                          ? 'text-emerald-600 dark:text-emerald-400'
                          : 'text-slate-400']"
                      />
                    </div>
                    <!-- Label -->
                    <p class="flex-1 text-sm font-medium text-slate-900 dark:text-slate-100">
                      {{ field.label }}
                    </p>
                    <!-- Toggle -->
                    <div class="flex items-center gap-2 shrink-0">
                      ...
                      >
                        {{ localForm.privacySettings[field.key] ? 'Visible' : 'Oculto' }}
                      </span>
                      <div
                        class="relative h-5 w-9 rounded-full transition-colors duration-200 shrink-0"
                        :class="localForm.privacySettings[field.key] ? 'bg-emerald-500' : 'bg-slate-300 dark:bg-slate-600'"
                      >
                        <div
                          class="absolute top-0.5 h-4 w-4 rounded-full bg-white shadow-sm transition-transform duration-200"
                          :class="localForm.privacySettings[field.key] ? 'translate-x-4' : 'translate-x-0.5'"
                        />
                      </div>
                    </div>
                  </div>
              ...
              <div>
                ...
                <p class="text-xs text-slate-400 dark:text-slate-500 mb-3 leading-relaxed">
                  Tu jefe directo siempre puede agregarte independientemente de esta configuración.
                </p>
                <div class="grid grid-cols-2 gap-2">
                  <button
                    v-for="opt in [
                      { value: true, label: 'Cualquiera', icon: 'pi-users', desc: 'Cualquiera puede agregarte' },
                      { value: false, label: 'Solo invitación', icon: 'pi-envelope-open', desc: 'Debés aceptar la invitación' },
                    ]"
                    :key="String(opt.value)"
                    type="button"
                    class="flex flex-col items-start gap-2 rounded-2xl border p-4 text-left transition"
                    :class="localForm.privacySettings.allowGroupInvites === opt.value
                    ...
                    @click="localForm.privacySettings.allowGroupInvites = opt.value"
                  >
                    <i
                      class="pi text-lg"
                      :class="[opt.icon, localForm.privacySettings.allowGroupInvites === opt.value
                        ? 'text-orange-500'
                        : 'text-slate-400 dark:text-slate-500']"
                    />
                    <div>
                      ...
                      <p class="text-[11px] text-slate-400 dark:text-slate-500 mt-0.5 leading-relaxed">
                        {{ opt.desc }}
                      </p>
                    </div>
                  </button>
          ...
          <!-- Footer -->
          <div class="px-6 pb-6 pt-4 shrink-0 border-t border-slate-100 dark:border-white/5">
            <p
            ...
            <div class="flex gap-2">
              ...
              >
                <i
                  class="pi"
                  :class="saveSuccess ? 'pi-check' : isSaving ? 'pi-spin pi-spinner' : 'pi-save'"
                />
        ...
        <Transition name="mfade">
          ...
          >
            ...
            >
              <div class="flex h-11 w-11 items-center justify-center rounded-full bg-amber-100 dark:bg-amber-900/40 mb-4">
                <i class="pi pi-exclamation-triangle text-amber-500 text-lg" />
              </div>
              <h4 class="text-sm font-semibold text-slate-900 dark:text-slate-50 mb-1">
                ¿Descartar cambios?
              </h4>
              <p class="text-xs text-slate-500 dark:text-slate-400 leading-relaxed mb-5">
                Tenés cambios sin guardar en tu perfil. Si salís ahora se perderán.
              </p>
              <div class="flex gap-2">
                <button
                  type="button"
...
<style scoped>
.mfade-enter-active, .mfade-leave-active { transition: opacity 0.15s ease; }
